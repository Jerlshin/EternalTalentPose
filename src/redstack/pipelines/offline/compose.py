...
#: Required ``golden_labels.csv`` header. ``cited_features`` is a
#: ``;``-separated list within its cell (CSV has no native list type).
_REVIEW_CSV_FIELDS: Final[tuple[str, ...]] = (
    "candidate_id",
    "tier",
    "reasoning",
    "reviewer",
...
_CSV_TRUE: Final[frozenset[str]] = frozenset({"true", "1", "yes"})


def _parse_review_row(row: Mapping[str, str], *, line_no: int) -> ReviewTag:
    """Parse one ``golden_labels.csv`` data row into a ``ReviewTag``.

    Raises:
        GoldLabelSeedMissingError: the row is malformed (bad int, empty
            required field) — surfaced with its 1-based data line for the
            reviewer to fix.
    """
    try:
        archetype_raw = row.get("archetype_id", "").strip()
        return ReviewTag(
            candidate_id=row["candidate_id"].strip(),
            tier=int(row["tier"]),
            reasoning=row["reasoning"],
            reviewer=row["reviewer"].strip(),
            archetype_id=int(archetype_raw) if archetype_raw else None,
            is_honeypot_suspect=row.get("is_honeypot_suspect", "").strip().lower()
            in _CSV_TRUE,
            is_borderline=row.get("is_borderline", "").strip().lower() in _CSV_TRUE,
            cited_features=tuple(
                f.strip()
                for f in row.get("cited_features", "").split(";")
                if f.strip()
            ),
        )
    except (KeyError, ValueError) as exc:
        raise GoldLabelSeedMissingError(
            f"golden_labels.csv data row {line_no} is malformed: {exc}"
        ) from exc
...
def _load_gold_label_seed(path: Path) -> GoldLabelSeed:
    ...
    ``path`` is a CSV with header :data:`_REVIEW_CSV_FIELDS` (one row per
    reviewer tag); ``archetype_id`` empty means ``None``, ``is_honeypot_suspect``
    / ``is_borderline`` are ``true``/``false``-style strings, and
    ``cited_features`` is a ``;``-separated list.

    Raises:
        GoldLabelSeedMissingError: ``path`` does not exist, or a data row is
            malformed. A missing file is *not* a code bug — O8 is
            human-in-the-loop labeling; the seed is a workspace output
            authored by reviewers, never synthesized here.
    """
    if not path.is_file():
        raise GoldLabelSeedMissingError(
    ...
    with path.open(encoding="utf-8", newline="") as handle:
        ...
        tags = tuple(
            _parse_review_row(row, line_no=line_no)
            for line_no, row in enumerate(reader, start=2)
        )
    if not tags:
        raise GoldLabelSeedMissingError(f"golden_labels.csv at {path} has no data rows")
    return GoldLabelSeed(tags=tags)
